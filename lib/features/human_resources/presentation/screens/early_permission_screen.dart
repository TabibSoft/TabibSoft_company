...
class _EarlyPermissionScreenState extends State<EarlyPermissionScreen> {
                          ...
                          children: [
                            ...
                            SizedBox(
                              ...
                              child: ElevatedButton(
                                onPressed: () =>
                                    _submitEarlyPermissionRequest(context),
  ...
  Future<void> _submitEarlyPermissionRequest(BuildContext context) async {
    if (exitTime == null || returnTime == null) {
      _showValidationMessage('يرجى تحديد وقت الخروج ووقت العودة');
      return;
    }
    ...
    if (endDate.isBefore(startDate) || endDate.isAtSameMomentAs(startDate)) {
      _showValidationMessage('وقت العودة يجب أن يكون بعد وقت الخروج');
      return;
    }

    final hoursRequested = endDate.difference(startDate).inMinutes;
    if (hoursRequested <= 0) {
      _showValidationMessage('يرجى تحديد مدة صحيحة للاستئذان');
      return;
    }

    final hours = (hoursRequested / 60).ceil();
    if (hours > 2) {
